...
public class MySqlCDDao extends AbstractMySqlMaterialDao<CD> implements CDDao {
    ...
    public CD save(CD cd) throws SQLException {
        ...
        final String sqlCd = "INSERT INTO cd (codigo_id, artista, genero, duracion, numero_canciones) VALUES (?, ?, ?, ?, ?)";
        ...
        try (Connection connection = ConnectionManager.getConnection()) {
            ...
            try {
                ...
                try (PreparedStatement materialStatement = connection.prepareStatement(sqlMaterial);
                    ...
                    materialStatement.setObject(3, cd.getUnidadesDisponibles());
                    materialStatement.executeUpdate();

                    cdStatement.setString(1, code);
                    cdStatement.setString(2, cd.getArtista());
                    cdStatement.setString(3, cd.getGenero());
                    cdStatement.setObject(4, cd.getDuracion());
                    cdStatement.setInt(5, cd.getNumeroCanciones());
    ...
    public void update(CD cd) throws SQLException {
        ...
        final String sqlCd = "UPDATE cd SET artista = ?, genero = ?, duracion = ?, numero_canciones = ? WHERE codigo_id = ?";
        ...
        try (Connection connection = ConnectionManager.getConnection()) {
            ...
            try {
                ...
                try (PreparedStatement materialStatement = connection.prepareStatement(sqlMaterial)) {
                    ...
                    materialStatement.setObject(2, cd.getUnidadesDisponibles());
                ...
                try (PreparedStatement cdStatement = connection.prepareStatement(sqlCd)) {
                    ...
                    cdStatement.setString(2, cd.getGenero());
                    cdStatement.setObject(3, cd.getDuracion());
                    cdStatement.setInt(4, cd.getNumeroCanciones());
                    cdStatement.setString(5, code);
    ...
    public CD findByCode(String code) throws SQLException {
        final String sql = "SELECT m.codigo, m.titulo, m.unidades_disponibles, c.artista, c.genero, c.duracion, c.numero_canciones "
    ...
    public List<CD> search(String text) throws SQLException {
        ...
        final String sql = "SELECT m.codigo, m.titulo, m.unidades_disponibles, c.artista, c.genero, c.duracion, c.numero_canciones "
                ...
                + "WHERE LOWER(m.codigo) LIKE ? OR LOWER(m.titulo) LIKE ? OR LOWER(c.artista) LIKE ? OR LOWER(c.genero) LIKE ? "
                + "OR CAST(c.duracion AS CHAR) LIKE ? OR CAST(c.numero_canciones AS CHAR) LIKE ? ORDER BY m.codigo DESC";
    ...
    private void createCtiTablesIfNeeded() throws SQLException {
        final String cdSql = """
            ...
            cdStatement.execute();
            addColumnIfNeeded(connection, "ALTER TABLE cd ADD COLUMN genero VARCHAR(80) NOT NULL DEFAULT 'Sin genero'");
            addColumnIfNeeded(connection, "ALTER TABLE cd ADD COLUMN duracion INT NOT NULL DEFAULT 1");
            createIndexIfNeeded(connection, "CREATE INDEX idx_cd_artista ON cd (artista)");
            createIndexIfNeeded(connection, "CREATE INDEX idx_cd_genero ON cd (genero)");
        } catch (SQLException ex) {
    ...
    private CD mapRow(ResultSet resultSet) throws SQLException {
        ...
        cd.setDuracion(resultSet.getInt("duracion"));
